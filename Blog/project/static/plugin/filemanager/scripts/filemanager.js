...
var getFileInfo = function(file){
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + file, function(data){
		...
		if(data['Code'] == 0){
			...
			if(data['File Type'] == "png" || data['File Type'] == "gif" || data['File Type'] == "jpg" || data['File Type'] == "jpeg" || data['File Type'] == "bmp"){
				$('#fileinfo').find('img').attr('src',data['Path']);
		...
		if(data){	
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					console.log(data);
					if(data[key]['File Type'] === 'png' || data[key]['File Type'] === 'jpg' || data[key]['File Type'] === 'gif' || data[key]['File Type'] === 'jpeg' || data[key]['File Type'] === 'bmp')
						result += '<li><div class="clip"><img src="' + data[key]['Path'] + '" width="' + scaledWidth + '" alt="' + data[key]['Path'] + '" /></div><p>' + data[key]['Filename'] + '</p>';
					else
